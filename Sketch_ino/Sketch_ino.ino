...
void set()
{
	...
	toff = For;
	savedE = (EleSupplyC - 0.3) * toff;
	savedM = EleRate * (EleSupplyC - 0.3) * EleSupplyV * toff / 1000;
	wFile(0, " ");
  	setP=millis();
	x = For * 60 * 60 * 1000;
	Serial.println(x);
...
void hUpdGot()
{
	...
	else
	{
		...
		if (GotData.substring(GotData.indexOf("Check") + 6, GotData.indexOf("SSID") - 1) == "1111")
		{
			...
			String w = GotData.substring(GotData.indexOf("PWD") + 4, GotData.indexOf("Price") - 1);
			EleRate = GotData.substring(GotData.indexOf("Price") + 6, GotData.indexOf("SupplyV") - 1).toInt();
			EleSupplyV = GotData.substring(GotData.indexOf("SupplyV") + 8, GotData.indexOf("SupplyC") - 1).toInt();
			EleSupplyC = GotData.substring(GotData.indexOf("SupplyC") + 8, GotData.indexOf("reset") - 1).toInt();
			File file = SPIFFS.open("/const.txt", "w");
			// Serial.println(GotData.substring(GotData.indexOf("Price") + 6, GotData.indexOf("ElectricSupplyC") - 1));
			...
			file.print("//EleR//");
			file.print(EleRate);
			file.print("//EleSupplyC//");
			file.print(EleSupplyC);
			file.print("//EleSupplyV//");
			file.print(EleSupplyV);
			file.print("//eend");
			file.close();
		}
...
void hStat()
{
	if (stat)
	{
		server.send(200, "text/plain", "Stat : on\n"+String(y)+" "+String(pos)+"\n"+String(y - scedP[0] * 60)+"\n"+String(y - scedP[1] * 60)+"\n"+String(y - scedP[2] * 60)+"\n"+String(y - scedP[3] * 60)+"\n"+String(y - scedP[4] * 60)+"\n"+String(y - scedP[5] * 60)+"\n"+String(y - scedP[6] * 60)+"\n"+String(y - scedP[7] * 60)+"\n"+String(y - scedP[8] * 60)+"\n"+String(y - scedP[9] * 60)+"\n"+String(y - scedP[10] * 60)+"\n"+String(y - scedP[11] * 60)+"\n"+String(y - scedP[12] * 60));
	}
	else
	{
		server.send(200, "text/plain", "Stat : off\n"+String(y)+" "+String(pos)+"\n"+String(y - scedP[0] * 60)+"\n"+String(y - scedP[1] * 60)+"\n"+String(y - scedP[2] * 60)+"\n"+String(y - scedP[3] * 60)+"\n"+String(y - scedP[4] * 60)+"\n"+String(y - scedP[5] * 60)+"\n"+String(y - scedP[6] * 60)+"\n"+String(y - scedP[7] * 60)+"\n"+String(y - scedP[8] * 60)+"\n"+String(y - scedP[9] * 60)+"\n"+String(y - scedP[10] * 60)+"\n"+String(y - scedP[11] * 60)+"\n"+String(y - scedP[12] * 60));
	}
}
void hConst()
{
...
void handleRequest()
{
  ...
  server.on("/reboot", [](){server.send(200,"text/plain","ok");delay(1000);ESP.reset();});
    ...
    flag = false;
		if (http.begin(client, "http://worldtimeapi.org/api/timezone/Asia/Kolkata"))
		{
			...
			if (httpCode > 0)
			{
				...
				date=
				x.substring(x.indexOf("datetime") + 19, x.indexOf("datetime") + 21)+"-"
				+x.substring(x.indexOf("datetime") + 16, x.indexOf("datetime") + 18)+"-"
				+x.substring(x.indexOf("datetime") + 11, x.indexOf("datetime") + 15);
...
void setup()
{
	...
	EleRate = s.substring(s.indexOf("EleR//") + 6, s.indexOf("//EleSupplyC")).toInt();
	EleSupplyC = s.substring(s.indexOf("EleSupplyC//") + 12, s.indexOf("//EleSupplyV")).toInt();
	EleSupplyV=s.substring(s.indexOf("EleSupplyV//") + 12, s.indexOf("//eend")).toInt();
	file.close();
...
void loop()
{
	...
	if (flag)
	{
		 y = timeScedM + millis() / (1000 * 60);
		for (int x = 0; x < 13; x++)
		{
			if ((y - scedP[x] * 60) > 0 &&(y - scedP[x] * 60) < 120 && (y - scedP[x+1] * 60) < 0)
			{
        pos=scedP[x];
				sced(x);
			}
